...
__cold static void compacting_fixup_meta(MDBX_env *env, meta_t *meta) {
  ...
  if (meta->geometry.first_unallocated != meta->geometry.now) {
    ...
    if (aligner) {
      const pgno_t aligned = pgno_align2sp_pgno(env, meta->geometry.first_unallocated + aligner -
...
retry_snap_meta:
  ...
  /* Copy the data */
  const size_t whole_size = pgno_align2sp_bytes(env, txn->geo.end_pgno);
...
__cold static int copy2fd(MDBX_txn *txn, mdbx_filehandle_t fd, MDBX_copy_flags_t flags) {
  ...
  const size_t buffer_size =
      pgno_align2sp_bytes(env, NUM_METAS) +
