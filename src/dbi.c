...
int mdbx_dbi_close(MDBX_env *env, MDBX_dbi dbi) {
  ...
  if (likely(rc == MDBX_SUCCESS && dbi < env->n_dbi)) {
    if (env->basal_txn && (env->dbs_flags[dbi] & DB_VALID) &&
       ...
       * скрытой/непрозрачной активности внутри транзакции потенциально живущей
       * в другом потоке. Другими словами, проблема может быть только при
       * неверном использовании API и если пользователь это допускает, то точно
       * не будет ожидать скрытых действий внутри транзакции, и поэтому этот
       * путь потенциально более опасен. */
      ...
      bailout_dirty_dbi:
        ...
        return MDBX_DANGLING_DBI;
      } else if (hazard != env->basal_txn && hazard == env->txn && hazard &&
                 (hazard->flags & MDBX_TXN_FINISHED) == 0 &&
                 hazard->signature == txn_signature &&
                 (dbi_state(hazard, dbi) &
                  (DBI_LINDO | DBI_DIRTY | DBI_CREAT)) > DBI_LINDO)
