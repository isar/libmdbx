...
__cold int mdbx_env_set_geometry(MDBX_env *env, intptr_t size_lower, intptr_t size_now, intptr_t size_upper,
  ...
  if (!env->dxb_mmap.base) {
    ...
    ENSURE(env, pgno_align2sp_bytes(env, new_geo.lower) == (size_t)size_lower);
    ENSURE(env, pgno_align2sp_bytes(env, new_geo.upper) == (size_t)size_upper);
    ENSURE(env, pgno_align2sp_bytes(env, new_geo.now) == (size_t)size_now);
