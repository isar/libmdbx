...
MDBX_NOTHROW_PURE_FUNCTION size_t bytes_align2sp_bytes(const MDBX_env *env, size_t bytes) {
  return ceil_powerof2(bytes, (env->ps > globals.sys_pagesize) ? env->ps : globals.sys_pagesize);
}

MDBX_NOTHROW_PURE_FUNCTION size_t pgno_align2sp_bytes(const MDBX_env *env, size_t pgno) {
  return ceil_powerof2(pgno2bytes(env, pgno), globals.sys_pagesize);
}

MDBX_NOTHROW_PURE_FUNCTION pgno_t pgno_align2sp_pgno(const MDBX_env *env, size_t pgno) {
...
__cold void update_mlcnt(const MDBX_env *env, const pgno_t new_aligned_mlocked_pgno, const bool lock_not_release) {
  for (;;) {
    ...
    eASSERT(env, pgno_align2sp_pgno(env, mlock_pgno_before) == mlock_pgno_before);
    eASSERT(env, pgno_align2sp_pgno(env, new_aligned_mlocked_pgno) == new_aligned_mlocked_pgno);
...
__cold void munlock_all(const MDBX_env *env) {
  munlock_after(env, 0, bytes_align2sp_bytes(env, env->dxb_mmap.current));
}
