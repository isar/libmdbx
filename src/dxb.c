...
__cold int dxb_resize(MDBX_env *const env, const pgno_t used_pgno, const pgno_t size_pgno, pgno_t limit_pgno,
  ...
  const size_t limit_bytes = pgno_align2sp_bytes(env, limit_pgno);
  const size_t size_bytes = pgno_align2sp_bytes(env, size_pgno);
  ...
  if (mresize_flags & (MDBX_MRESIZE_MAY_UNMAP | MDBX_MRESIZE_MAY_MOVE)) {
    ...
    if ((env->flags & MDBX_WRITEMAP) && env->lck->unsynced_pages.weak) {
      ...
      rc = osal_msync(&env->dxb_mmap, 0, pgno_align2sp_bytes(env, used_pgno), MDBX_SYNC_NONE);
      if (unlikely(rc != MDBX_SUCCESS))
        goto bailout;
    }
  }
...
__cold int dxb_set_readahead(const MDBX_env *env, const pgno_t edge, const bool enable, const bool force_whole) {
  ...
  size_t offset = toggle ? 0 : pgno_align2sp_bytes(env, (prev_edge < edge) ? prev_edge : edge);
  offset = (offset < limit) ? offset : limit;

  size_t length = pgno_align2sp_bytes(env, (prev_edge < edge) ? edge : prev_edge);
...
__cold int dxb_setup(MDBX_env *env, const int lck_rc, const mdbx_mode_t mode_bits) {
  ...
  if ((env->flags & MDBX_RDONLY)    /* readonly */
    ...
    if (bytes_align2sp_bytes(env, env->geo_in_bytes.upper) != pgno2bytes(env, header.geometry.upper) ||
        bytes_align2sp_bytes(env, env->geo_in_bytes.lower) != pgno2bytes(env, header.geometry.lower) ||
        bytes_align2sp_bytes(env, env->geo_in_bytes.shrink) != pgno2bytes(env, pv2pages(header.geometry.shrink_pv)) ||
        bytes_align2sp_bytes(env, env->geo_in_bytes.grow) != pgno2bytes(env, pv2pages(header.geometry.grow_pv))) {
      ...
      expected_filesize = pgno_align2sp_bytes(env, header.geometry.now);
      ...
      env->geo_in_bytes.now = pgno_align2sp_bytes(env, header.geometry.now);
    }
  ...
  ENSURE(env, pgno_align2sp_bytes(env, header.geometry.now) == env->geo_in_bytes.now);
  ...
  if (globals.runtime_flags & MDBX_DBG_DUMP) {
    const size_t meta_length_aligned2os = pgno_align2sp_bytes(env, NUM_METAS);
...
int dxb_sync_locked(MDBX_env *env, unsigned flags, meta_t *const pending, troika_t *const troika) {
  ...
  if (flags & txn_shrink_allowed) {
    ...
    if (prev_discarded_pgno < pending->geometry.first_unallocated)
      ...
      const size_t discard_edge_pgno = pgno_align2sp_pgno(env, largest_pgno);
      if (prev_discarded_pgno >= discard_edge_pgno + env->madv_threshold) {
        const size_t prev_discarded_bytes = pgno_align2sp_bytes(env, prev_discarded_pgno);
        ...
        if (prev_discarded_bytes > discard_edge_bytes) {
          ...
          munlock_after(env, discard_edge_pgno, bytes_align2sp_bytes(env, env->dxb_mmap.current));
          const uint32_t munlocks_before = atomic_load32(&env->lck->mlcnt[1], mo_Relaxed);
#if defined(MADV_DONTNEED)
      ...
      if (pending->geometry.shrink_pv && pending->geometry.now - pending->geometry.first_unallocated >
        ...
        if (pending->geometry.now > largest_pgno &&
          ...
          const pgno_t aligned =
              pgno_align2sp_pgno(env, (size_t)with_stockpile_gap + aligner - with_stockpile_gap % aligner);
  ...
  if (atomic_load64(&env->lck->unsynced_pages, mo_Relaxed)) {
    ...
    if (flags & MDBX_WRITEMAP) {
      ...
      (void)sync_op;
#endif /* MDBX_ENABLE_PGOP_STAT */
      rc = osal_msync(&env->dxb_mmap, 0, pgno_align2sp_bytes(env, pending->geometry.first_unallocated), mode_bits);
    } else {
#if MDBX_ENABLE_PGOP_STAT
  ...
  if (flags & MDBX_WRITEMAP) {
    ...
    if (!env->incore) {
      if (!MDBX_AVOID_MSYNC) {
        ...
        env->lck->pgops.msync.weak += 1;
#endif /* MDBX_ENABLE_PGOP_STAT */
        rc = osal_msync(&env->dxb_mmap, 0, pgno_align2sp_bytes(env, NUM_METAS),
